    ...
    if (!email | !password) {
      ...
      if (!auth_user) {
        ...
        if (!verify) {
          ...
          return res
            .status(200)
            .json({ token: `Bearer ${token}`, user: auth_user });
        }
      }
    }
  } catch (error) {
...
const signup = async (req, res) => {
  try {
    ...
    if (!username | !password | !email) {
      ...
      console.log(
        "email includes manuu.edu.in",
        email.includes("@manuu.edu.in")
      );
      const includeManuu = email.includes("@manuu.edu.in");
      if (includeManuu) {
        ...
        return res.status(409).json({ message: "register with manuu email" });
